...
val_mse <- rep(NA, 5)
for(i in seq_len(5)) {
  glass_val_train <- glass %>% slice(-crossval_val_fold_inds[[i]])
  glass_val_val <- glass %>% slice(crossval_val_fold_inds[[i]])
  glass_val <- glass %>% slice(crossval_val_fold_inds[[i]])

  y_hat_trans <- predict(knn_fit, newdata = glass_val)
  val_mse[i] <- mean((glass_val$fev - y_hat_trans)^2)
}
mean(val_mse)
```
